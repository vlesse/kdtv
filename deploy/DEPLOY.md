...
| 没改的 | 为什么 |
| --- | --- |
| 包名 `com.wewatch.tv` | 包名+签名 = 「这台盒子上的这个应用」的身份。改了对已装机的电视来说是**另一个应用**：老的卸不掉、新的是第二个图标，而且 ANDROID_ID 会变，全部设备要重新注册。 |
| JS 桥 `window.WeWatchShell` | 这是**已经装在客人电视上那个 APK** 注入的名字。网页这边改了而盒子还是旧包，设备号就拿不到，每个内容请求都会 403。改它等于要求每台电视先换包 —— 而这正是薄壳架构要避免的事。 |
| 签名证书 `CN=WeWatch TV` | 证书主题改了就是另一把钥匙，装过的机器一台也升不上去。 |
